...
| ✅ | **`max_rate_rad_per_s=3.0` RE-VERIFIED against the fixed baseline, 2026-09-22** — same conclusion as the original (confounded) tuning holds, now for the right reason. Pushed at 7.0 rad/s (large enough to actually stress the recovery), compared 2.0/3.0/4.0 at t=4.0s: 0.81°/0.56°/0.71° from the true ~0.5° resting point — 3.0 still the local optimum. The story changed, not the number: this isn't "prevents overshoot" (there wasn't real overshoot once the confound was removed) — it's "converges fastest to the true resting point." All three traces were smooth and monotonic, no oscillation at any value; `ankle_kp=0.6`/`ankle_kd=0.05` visibly stable throughout the same traces, not separately re-swept. See the RESOLVED note below "Integral term attempt" for the full account. |
...
2026-09-15):** `max_rate_rad_per_s=3.0` re-verified 2026-09-22 against
the fixed baseline (7.0 rad/s push, compared 2.0/3.0/4.0 at t=4.0s:
0.81°/0.56°/0.71° from the true resting point — 3.0 still the local
optimum, same conclusion as before, now for the right reason: fastest
convergence to the true resting point, not "prevents overshoot," since
there wasn't real overshoot once the confound was removed). `ankle_kp`/
`ankle_kd` not separately re-swept, but visibly stable and non-
oscillating across all of that same re-verification's traces. The
qualitative lessons about individual parameters not directly
re-verified (e.g., large `ankle_kd` jumps causing oscillatory
instability) likely still hold as general PID-tuning facts, but treat
any *specific* untested comparison as provisional until it's actually
...
- **2026-09-22 (continued)** — Re-verified `max_rate_rad_per_s` against the fixed baseline, per the previous entry's own recommendation. Pushed at 7.0 rad/s (large enough to actually stress recovery), compared 2.0/3.0/4.0: peaks nearly identical (~8.6°) across all three, but at t=4.0s, 3.0 was closest to the true resting point (0.56° vs 2.0's 0.81° and 4.0's 0.71°) — same conclusion as the original (confounded) tuning, now confirmed for the right reason (fastest convergence, not "prevents overshoot," since none of the three showed real overshoot once the confound was gone). No code change — `max_rate_rad_per_s` stays at 3.0. `ankle_kp`/`ankle_kd` not separately re-swept but visibly stable across the same traces.
