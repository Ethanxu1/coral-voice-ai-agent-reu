...
* Load and network conditions are uncontrolled. Transcription runs locally on the laptop's CPU (faster-whisper, int8), so its duration depends on what else the Mac is doing — the vision server and the frontend share the same machine — and the Mac→Pi hop depends on local network conditions. Neither was held constant or recorded per trial.
