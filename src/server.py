...
from robot.angle_utils import rad_to_servo_units, servo_units_to_rad, speed_to_duration_ms
from robot.hardware_angle_utils import hardware_units_to_rad
from robot.hardware_controller import AiNexHardwareController
...
from collision.stability_checker import StabilityChecker
from simulator.mujoco_sim import COMMAND_MAP, execute_command
from state import (
    StateManager,
)
...
def get_router_prompt() -> str:
    ...
    if _router_prompt_cache is None:
        _router_prompt_cache = (Path(__file__).parent / "llm" / "prompts" / "router.md").read_text(encoding="utf-8")
...
def _reexec_under_mjpython_if_needed() -> None:
    ...
    argv = sys.argv if sys.argv and Path(sys.argv[0]).exists() else ["-m", "server"]
