...
async def map_features(leg_mode: str = "buckets"):
    ...
    query param (default "buckets") selects how the *legs* are driven — see
    leg_modes.py:
      "buckets"   classify knee/ankle bend depth and hip yaw into discrete
                  stances (low/stand/high, in/stand/out) and snap to each
                  bucket's fixed target (default).
      "retarget"  live pelvis-frame hip pitch/roll + knee bend.
