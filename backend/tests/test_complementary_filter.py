...
using synthetic accel/gyro sequences. They do NOT and cannot verify that
the axis convention (which raw index means "roll" vs "pitch", and which
sign) actually matches the real IMU's physical mounting — that needs the
real hardware, see the module docstring and
...
class TestValidation:
    ...
    def test_dt_must_be_positive(self):
        ...
        with pytest.raises(ValueError):
            f.update(accel_g=(0, 0, 1), gyro_deg_s=(0, 0, 0), dt=0.0)
        with pytest.raises(ValueError):
            f.update(accel_g=(0, 0, 1), gyro_deg_s=(0, 0, 0), dt=-0.01)
...
class TestAtRest:
    def test_upright_at_rest_stays_near_zero(self):
        f = ComplementaryFilter()
        for _ in range(50):
            reading = f.update(accel_g=(0.0, 0.0, 1.0), gyro_deg_s=(0.0, 0.0, 0.0), dt=0.01)
    ...
    def test_reset_clears_accumulated_state(self):
        ...
        for _ in range(20):
            f.update(accel_g=(0.0, 0.0, 1.0), gyro_deg_s=(50.0, 0.0, 0.0), dt=0.01)
        f.reset()
        reading = f.update(accel_g=(0.0, 0.0, 1.0), gyro_deg_s=(0.0, 0.0, 0.0), dt=0.01)
        assert reading.roll_rad == pytest.approx(0.0, abs=1e-9)
...
class TestConvergence:
    def test_converges_toward_accel_tilt_when_gyro_reports_no_rotation(self):
        ...
        # A tilt where the up-vector reads (sin(20deg), 0, cos(20deg)) on
        # the roll axis — i.e. accel implies roll = 20 degrees.
        target = math.radians(20)
        accel = (0.0, math.sin(target), math.cos(target))
        reading = None
        for _ in range(300):
    ...
    def test_pure_consistent_rotation_tracks_the_analytic_angle(self):
        ...
        for _ in range(steps):
            ...
            accel = (0.0, math.sin(true_angle), math.cos(true_angle))
...
class TestRateConversion:
    def test_gyro_degrees_per_second_converted_to_radians(self):
        f = ComplementaryFilter()
        reading = f.update(accel_g=(0.0, 0.0, 1.0), gyro_deg_s=(90.0, 180.0, 0.0), dt=0.01)
...
class TestStatefulness:
    def test_state_persists_and_accumulates_across_calls(self):
        ...
        first = f.update(accel_g=(0.0, 0.0, 1.0), gyro_deg_s=(100.0, 0.0, 0.0), dt=0.01)
        second = f.update(accel_g=(0.0, 0.0, 1.0), gyro_deg_s=(100.0, 0.0, 0.0), dt=0.01)
