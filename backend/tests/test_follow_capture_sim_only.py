...
from app.services.intent import SavePoseDialog, try_handle_system_intent
...
def _fake_follow_controller():
    fake = _FakeFollowController()
    prev = state.follow_controller
    state.follow_controller = fake
    yield fake
    state.follow_controller = prev


@pytest.mark.anyio
async def test_follow_start_forwards_sim_only(_fake_follow_controller):
    dialog = SavePoseDialog()
